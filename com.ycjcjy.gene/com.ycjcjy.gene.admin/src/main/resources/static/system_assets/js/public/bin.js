 ...
 *switch按钮切换值替换成0/1
 */
$('input[data-am-switch]').on('switchChange.bootstrapSwitch init.bootstrapSwitch', function (event,state) {
    var checked = $(this).attr('checked') === 'checked';
    checked = (typeof(state) == 'undefined')?checked:state;
    if(checked==true){
        $(this).val("1");
            ...
            if(options.type == "folder" || typeof(options.type) == "undefined"){
                if(typeof(options.childList) == "object"){
                    ...
                    doPost(url,{roleId:roleId},function(res){
                        ...
                        $('#PrTree-template').tree('selectItemSafe', $('.selected-item'));
                    })
                }
 ...
 */
function doPost(url,param,completeHandler,beforeSuccess) {
    var aBeforeSuccess=function () {};
    if(beforeSuccess!=undefined){
        aBeforeSuccess=beforeSuccess;
    }
    url = addCtxToUrl(url);
    $.AMUI.progress.start();
